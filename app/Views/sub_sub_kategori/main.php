                                        ...
                                        <div class="modal-body">
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="sub_kategori" name="sub_kategori">
                                                    ...
                                                    <?php
                                                    $sub_kategori = model('SubKategori')->findAll();
                                                    foreach ($sub_kategori as $v) :
                                                    ?>
                                                    <option value="<?= $v['id'] ?>"><?= $v['nama_kategori'] ?> - <?= $v['nama'] ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                                <div class="invalid-feedback" id="invalid_sub_kategori"></div>
                                            </div>
                                            <div class="mb-3">
        ...
        columns: [
            {
                ...
                name: 'nama_kategori',
                data: null,
                render: data => `${data.nama_kategori} - ${data.nama_sub_kategori}`,
            }, {
                name: 'nama',
                data: 'nama',
            }, {
...
function renderOpsi(data) {
    const sub_kategori = <?= json_encode(model('SubKategori')->findAll()) ?>;
                ...
                <form id="form_${data.id}">
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="sub_kategori" name="sub_kategori">
                                <option value="">Pilih</option>
                                ${sub_kategori.map(item => `<option value="${item.id}" ${item.id == data.id_sub_kategori ? 'selected' : ''}>${item.nama_kategori} - ${item.nama}</option>`).join('')}
                            </select>
                            <div class="invalid-feedback" id="invalid_sub_kategori"></div>
                        </div>
                        <div class="mb-3">
