                    ...
                    <form id="form">
                        <div class="mb-3">
                            ...
                            <div class="col-12 col-md-6 col-lg-5 col-xxl-4 position-relative">
                                <img src="<?= webFiles($base_name, $data['gambar'], true, $data['updated_at']) ?>" class="w-100 cover-center" id="frame_gambar">
                        ...
                        <div class="mb-3">
                            ...
                            <select id="produk" name="produk">
                                ...
                                foreach ($produk as $v) :
                                    $selected = ($v['id'] == $data['id_produk']) ? 'selected' : '';
                                ?>
                                <option value="<?= $v['id'] ?>" <?= $selected ?>><?= $v['nama_kategori'] ?> - <?= $v['nama'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="nama" name="nama" placeholder="Masukkan nama varian" value="<?= $data['nama'] ?>">
                            <div class="invalid-feedback" id="invalid_nama"></div>
                        </div>
                        <div class="mb-3">
                            <label for="sku" class="form-label">SKU</label>
                            <input type="text" class="form-control" id="sku" name="sku" placeholder="Masukkan sku" value="<?= $data['sku'] ?>" oninput="this.value = this.value.toUpperCase().replace(/[^A-Z0-9]/g, '')">
                            <div class="invalid-feedback" id="invalid_sku"></div>
                        </div>
                        <div class="mb-3">
                            <label for="harga_pokok" class="form-label">Harga Pokok</label>
                            <input type="text" inputmode="numeric" class="form-control" id="harga_pokok" name="harga_pokok" value="<?= dotsNumber(abs($data['harga_pokok'])) ?>" placeholder="Masukkan harga pokok" oninput="this.value = dotsNumber(this.value)">
                            <div class="invalid-feedback" id="invalid_harga_pokok"></div>
                        </div>
                        <div class="mb-3">
                            <label for="biaya_produk" class="form-label">Biaya Produk</label>
                            <input type="text" inputmode="numeric" class="form-control" id="biaya_produk" name="biaya_produk" value="<?= dotsNumber(abs($data['biaya_produk'])) ?>" placeholder="Masukkan biaya produk" oninput="this.value = dotsNumber(this.value)">
                            <div class="invalid-feedback" id="invalid_biaya_produk"></div>
                        </div>
                        <div class="mb-3">
                            <label for="harga_jual" class="form-label">Harga Jual</label>
                            <input type="text" inputmode="numeric" class="form-control" id="harga_jual" name="harga_jual" value="<?= dotsNumber(abs($data['harga_jual'])) ?>" placeholder="Masukkan harga jual" oninput="this.value = dotsNumber(this.value)">
...
form.addEventListener('submit', function(event) {
    ...
    const endpoint = '<?= $base_api ?>update/<?= encode($data['id']) ?>';
